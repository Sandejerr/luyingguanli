<template>
  <div>
    <el-form class="detail-form-content" ref="ruleForm" :model="ruleForm" label-width="auto">
      <el-row>
        <el-col :span="12">
          <el-form-item v-if="flag == 'chujiguanli'" label='初级管理姓名' prop="chujiguanliName">
            <el-input v-model="ruleForm.chujiguanliName" placeholder='初级管理姓名' clearable></el-input>
          </el-form-item>
        </el-col>

        <el-col :span="12">
          <el-form-item v-if="flag == 'chujiguanli'" label='初级管理手机号' prop="chujiguanliPhone">
            <el-input v-model="ruleForm.chujiguanliPhone" placeholder='初级管理手机号' clearable></el-input>
          </el-form-item>
        </el-col>

        <el-col :span="12">
          <el-form-item v-if="flag == 'chujiguanli'" label='初级管理身份证号' prop="chujiguanliIdNumber">
            <el-input v-model="ruleForm.chujiguanliIdNumber" placeholder='初级管理身份证号' clearable></el-input>
          </el-form-item>
        </el-col>

        <el-col :span="12">
          <el-form-item v-if="flag == 'chujiguanli'" label='初级管理头像' prop="chujiguanliPhoto">
            <file-upload tip="点击上传照片" action="file/upload" :limit="3" :multiple="true"
              ...
              @change="chujiguanliPhotoUploadChange"></file-upload>
          </el-form-item>
        </el-col>
        <el-col :span="12">
          <el-form-item v-if="flag == 'chujiguanli'" label='电子邮箱' prop="chujiguanliEmail">
            <el-input v-model="ruleForm.chujiguanliEmail" placeholder='电子邮箱' clearable></el-input>
          </el-form-item>
        </el-col>
        ...
        <el-col :span="12">
          <el-form-item v-if="flag != 'users' && (1 == 2 || flag == 'yonghu'
            || flag == 'chujiguanli')" label="性别" prop="sexTypes">
...
export default {
  ...
  methods: {
    ...
    onUpdateHandler() {
      if ((!this.ruleForm.chujiguanliName) && 'chujiguanli' == this.flag) {
        this.$message.error('初级管理姓名不能为空');
        return
      }

      if ('chujiguanli' == this.flag && this.ruleForm.chujiguanliPhone && (!isMobile(this.ruleForm.chujiguanliPhone))) {
        this.$message.error(`手机应输入手机格式`);
        return
      }
      if ((!this.ruleForm.chujiguanliIdNumber) && 'chujiguanli' == this.flag) {
        this.$message.error('初级管理身份证号不能为空');
        return
      }

      if ((!this.ruleForm.chujiguanliPhoto) && 'chujiguanli' == this.flag) {
        this.$message.error('初级管理头像不能为空');
        return
      }

      if ('chujiguanli' == this.flag && this.ruleForm.chujiguanliEmail && (!isEmail(this.ruleForm.chujiguanliEmail))) {
